...
def predict_on_test(model, market: str, scaler_stats, seq_len: int) -> pd.DataFrame:
    ...
    all_preds_inv = all_preds * price_range + price_min
    actual_inv    = y_te * price_range + price_min
...
def train_bilstm(market: str = "PJM", seq_len: int = SEQ_LEN):
    ...
    os.makedirs(config.REPORT_DIR, exist_ok=True)
    save_path = os.path.join(config.MODEL_DIR, f"bilstm_no_arcsinh_{market.lower()}.keras")
    ...
    pred_path = os.path.join(config.REPORT_DIR, f"bilstm_no_arcsinh_preds_{market.lower()}.csv")
    ...
    meta_path = os.path.join(config.MODEL_DIR, f"bilstm_no_arcsinh_{market.lower()}_meta.joblib")
