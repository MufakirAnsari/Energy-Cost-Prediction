"""
step_08_conformal.py  [DEPRECATED — use step_10_conformal.py]
==============================================================
This is a SUPERSEDED stub. The active CQR script is:
    step_10_conformal.py  (corrected Romano et al. quantile formula,
                           dedicated calibration set, full audit-clean)

Do not run this file directly. It exists for historical reference only.
==============================================================

ORIGINAL DOCSTRING (archived):
step_08_conformal.py
====================
...
def run_conformal(market: str = "PJM"):
    ...
    # Finite-sample guarantee requires (1-α)(1 + 1/n)-th quantile
    n_cal = len(y_cal)
    alpha = 1.0 - config.NOMINAL_COVERAGE   # = 0.10 for 90% coverage
    correction_level = np.ceil((1 - alpha) * (1 + 1 / n_cal)) / (1 + 1 / n_cal)
    correction = np.quantile(nonconf_scores, correction_level)
    print(f"  Nonconformity correction: {correction:.4f} $/MWh")
